...
public final class PageRank {
    ...
    public static JavaPairRDD<Integer, Double> sparkPageRank(
        ...
        final JavaPairRDD<Integer, Double> newRanks = sites.join(ranks).flatMapToPair(kv -> {
            ...
            while (iterator.hasNext()) {
                ...
                result.add(new Tuple2<>(target, currentRank / edges.getNEdges()));
            }

            return result;
        });

        return newRanks.reduceByKey(Double::sum).mapValues(value -> 0.15 + 0.85 * value);
    }
}
